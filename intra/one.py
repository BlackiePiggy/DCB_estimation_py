...
def format_sites_info(sites_info: Dict) -> Dict:
    ...
    doys = np.array(sites_info['doy'], dtype='float64').reshape(-1, 1)
    coords = np.array(sites_info['coor'], dtype='float64')

    return np.array([(names, doys, coords)],
                    dtype=[('name', 'O'), ('doy', 'O'), ('coor', 'O')])

def read_rinex(r_ipath: str, r_opath: str, signal_P1: str, signal_P2: str) -> Dict:
    ...
    for obsn in obs_files:
        try:
            ...
            obs, coor, year, doy = read_rinex_v3(fname, signal_P1, signal_P2)

            if np.all(obs['P1'] == 0) or np.all(obs['P2'] == 0):
...
def read_rinex_v3(path: str, signal_P1: str, signal_P2: str) -> Tuple[Dict, List, int, int]:
...
def read_observation_values(data: str, G_signal_name: List[str],
                          signal_P1: str, signal_P2: str) -> Dict[str, float]:
    """Parse observation values from RINEX data line."""
    target_signals = [signal_P1, signal_P2]
...
Sites_Info = read_rinex(r_ipath, r_opath, signal_P1, signal_P2)
